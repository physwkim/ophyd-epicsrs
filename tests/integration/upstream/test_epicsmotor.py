...
def test_calibration(motor):
    ...
    time.sleep(0.2)  # let monitor update after offset reset

    # Calibration
    old_position = motor.position
    expected_offset = 10 - motor.position
    motor.set_current_position(10)
    time.sleep(0.2)  # let monitor update after calibration
...
def test_homing_forward(motor):
    ...
    # MotorSim (C EPICS) is unable to execute homing, but epics-rs
    # SimMotor supports homing and succeeds.
